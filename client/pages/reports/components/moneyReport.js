...
export default class moneyReport extends Component {
  static displayName = 'moneyReport';
  ...
  getMoneyReport = async (value, year) => {
    Axios.post('/api/getMoneyReport', {
      value, year, type: 'money',
    }).then(res => {
  ...
  onChange = (v) => {
    if (v === '0') {
      this.getMoneyReport(0);
    } else {
      this.getMoneyReport(1, v);
    }
  };
    ...
    return (
      <IceContainer>
        <div style={styles.formContent}>
          <h2 style={styles.formTitle}>金额统计
            {/* <YearPicker /> */}
